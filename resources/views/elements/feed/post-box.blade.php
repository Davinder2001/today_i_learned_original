<div class="post-box post-box-container-section {{ isset($is_visited) && $is_visited ? 'visited_post' : '' }}"
    data-postID="{{ $post->id }}">
                ...
                <div class="d-flex post-header-flex-area">
                    <div class="post-details pl-2 w-100 post-details-heading-wrapper{{ $post->is_pinned ? '' : '' }}">
                        <div class="d-flex justify-content-between post-header-justify top-header-post-justify">
                            <div class="name-and-time-container d-flex">
                                <div class="text-bold post-url-image post_author">
                                    <!--<span class="post_section"></span>-->
                                    <a href="{{ route('profile', ['username' => $post->user->username]) }}"
                                        class="text-dark-r">Posted by {{ $post->user->name }} / </a>
                                </div>
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        @if (Auth::check())
                                            @if (!PostsHelper::isPostHidden($post->id))
                                                <a class="dropdown-item hide-button" href="javascript:void(0);"
                                                    onclick="Post.togglePostHide({{ $post->id }}, 'hide');"> Hide
                                                    this
                                                    post</a>
                                            @else
                                                <a class="dropdown-item hide-button" href="javascript:void(0);"
                                                    onclick="Post.togglePostHide({{ $post->id }}, 'unhide');">Unhide</a>
                                            @endif
                                        @else
                                            <div class="without-login-three-dot-links">
                                                <a href="{{ route('login') }}" class="dropdown-item">Hide</a>
                                            </div>
                                        @endif
                                        <div class="post-save-btn">
                                            @if (Auth::check())
                                                @if (!PostsHelper::isPostSaved($post->id))
                                                    <a class="dropdown-item save-button" href="javascript:void(0);"
                                                        onclick="Post.togglePostSave({{ $post->id }}, 'save');">
                                                        Save
                                                    </a>
                                                @else
                                                    <a class="dropdown-item save-button" href="javascript:void(0);"
                                                        onclick="Post.togglePostSave({{ $post->id }}, 'unsave');">
                                                        Unsave
                                                    </a>
                                                @endif
                                            @else
                                                <div class="without-login-three-dot-links">
                                                    <a href="{{ route('login') }}" class="dropdown-item">Save</a>
                                                </div>
                                            @endif
                                        ...
                                        @else
                                            <div class="without-login-three-dot-links">
                                                <a href="{{ route('login') }}" class="dropdown-item">Report</a>
                                            </div>
                                        @endif
                                        @if (Auth::check() && Auth::user()->id == $post->user->id)
                                            <div class="dropdown-divider"></div>
                ...
                <div class="post-catogiry-section">
                    ...
                    @if ($post->interests->isNotEmpty())
                        @foreach ($post->interests as $interest)
                            <div class='svg-suggestion-wrapper'>
                                ...
                                <p class='post-tag'>{{ $interest->name }}</p>
                            </div>
                        @endforeach
                    @endif
                </div>
            ...
            <div class="post-footer mt-3 pl-3 pr-3 post-bottom-footer">
                <div class="footer-actions d-flex justify-content-between">
                    <div class="d-flex footer-icon-flex-wrap">
                                ...
                                <div class='comment-wrapper'>
                                    ...
                                    <span class="ml-2-h d-none d-lg-block padding-remover">
                                        <a onclick="window.location.href = '{{ Route::currentRouteName() != 'posts.get' ? route('posts.get', ['post_id' => $post->id, 'username' => $post->user->username]) : '#comments' }}';"
                                            ...
                                            <strong
                                                class="post-comments-label-count post-span-tag">{{ count($post->comments) }}</strong>
                                ...
                                <div class="post-share-btn">
                                    @if (Auth::check())
                                        @if (!PostsHelper::isPostShared($post->id))
                                            <a class="dropdown-item share-button" href="javascript:void(0);"
                                                onclick="Post.togglePostShare({{ $post->id }}, 'Share');">
                                                Share
                                            </a>
                                        @else
                                            <a class="dropdown-item share-button" href="javascript:void(0);"
                                                onclick="Post.togglePostShare({{ $post->id }}, 'Already Shared');">
                                                Share
                                            </a>
                                        @endif
                                    @else
                                    <a class="dropdown-item share-button" href="{{route('login')}}">
                                                Share
                                            </a>
                                    @endif
                                </div>
                        ...
                        <div class="post-learned-btn">
                            @if (Auth::check())
                                @if (!PostsHelper::isPostLearned($post->id))
                                    <a class="dropdown-item learned-btn" href="javascript:void(0);"
                                        onclick="Post.togglePostLearned({{ $post->id }}, 'learned');">
                                        Learned
                                    </a>
                                @else
                                @endif
